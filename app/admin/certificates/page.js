...
export default function AdminCertificatesPage() {
    ...
    const load = () => { setLoading(true); fetch('/api/certificates').then(r => r.json()).then(d => { setItems(Array.isArray(d) ? d : []); setLoading(false); }).catch(() => setLoading(false)); };
    useEffect(load, []);

    const save = async () => {
        setSaving(true);
        const isNew = modal === 'new';
        const res = await fetch(isNew ? '/api/certificates' : `/api/certificates/${modal.id}`, { method: isNew ? 'POST' : 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
        setSaving(false);
        ...
        <div style={{ padding: '2rem 2.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
                <div>
                    ...
                    <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--text-primary)' }}>Certificates</h1>
                </div>
            ...
            {loading ? <div style={{ display: 'flex', justifyContent: 'center', padding: '4rem' }}><Loader2 size={28} style={{ color: 'var(--accent-cyan)', animation: 'spin 1s linear infinite' }} /></div>
                : items.length === 0 ? <div className="card-base" style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>No certificates yet.</div>
                    : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.25rem' }}>
                        {items.map(item => (
                            <div key={item.id} className="card-base" style={{ padding: '1.25rem' }}>
                                <p style={{ fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.25rem' }}>{item.title}</p>
                                <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem' }}>{item.organization}</p>
                                <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginBottom: '1rem' }}>{item.date}</p>
                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                    ...
                                    {item.verifyUrl && <a href={item.verifyUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', padding: '0.4rem 0.6rem', borderRadius: '8px', background: 'var(--accent-glow-cyan)', border: '1px solid rgba(34,211,238,0.2)', color: 'var(--accent-cyan)', textDecoration: 'none' }}><ExternalLink size={13} /></a>}
                                </div>
                            </div>
                        ))}
                    </div>}
                ...
                {modal && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setModal(null)}
                        ...
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} onClick={e => e.stopPropagation()}
                            className="card-base" style={{ width: '100%', maxWidth: 480, padding: '2rem', maxHeight: '90vh', overflowY: 'auto' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                <h2 style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{modal === 'new' ? 'Add Certificate' : 'Edit Certificate'}</h2>
